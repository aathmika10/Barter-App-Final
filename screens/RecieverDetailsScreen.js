...
getUserDetails=(userId)=>{
    db.collection("users").where('wmail_id','==',userId).get()
    .then((snapshot)=>{
        snapshot.forEach((doc)=>{
            this.setState({
...
addNotification=()=>{
    var message=this.state.userName+"has shown interest in donating the thing"
...
<View style={styles.container}>
    ...
    <View style={{flex:0.3}}>
        ...
        titleStyle={{fontSize:20}}>
           ...
           <Card>
                <Text style={{fontWeight:'bold'}}>Name:{this.state.thingName}</Text>
            </Card>
            <Card>
